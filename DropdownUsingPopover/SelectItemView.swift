...
class SelectItemView: UIView{
    ...
    */
    ...
    lazy var dropdownImg: TriangleView = {
        let imageView = TriangleView()
        //imageView.image = UIImage(named: "dropdown_btn")
        imageView.translatesAutoresizingMaskIntoConstraints = false
        self.addSubview(imageView)
    ...
    func setup(icon:UIImage?, itemTxt:String, txtColor:UIColor, borderColor:UIColor, triangleColor:UIColor){
  
        if let icon = icon{
            self.itemImg.image = icon
        }else{
            self.itemImg.isHidden = true
        }
        self.itemNameLabel.text = "\(itemTxt)" 
        
        self.itemNameLabel.textColor = txtColor
        self.layer.borderColor = borderColor.cgColor
        self.dropdownImg.triangleColor = triangleColor
    }
